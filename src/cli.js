...
function buildProjectArgs(action, root, configuration = {}) {
  const args = ["project", action, "--project", root, ...(configuration.defaultArgs || [])];
  if (action === "run" && configuration.runArgs?.length) args.push("--", ...configuration.runArgs);
  return args;
}
...
class WioCli {
  ...
  async file(action, document, extra = [], options = {}) {
    ...
    args.push(...extra);
    return this.run(args, path.dirname(document.uri.fsPath), { ...options, activeFile: document.uri.fsPath });
  }

  async project(action, root, options = {}) {
    const config = this.configuration();
    return this.run(buildProjectArgs(action, root, config), root, options);
  }

  async doctor(root) {
    const config = this.configuration();
    return this.run(["env", "doctor", ...(root ? ["--wio-root", root] : []), ...config.defaultArgs], root);
  }
